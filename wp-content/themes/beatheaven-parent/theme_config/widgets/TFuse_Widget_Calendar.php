...
class TFuse_Widget_Calendar extends WP_Widget {
	...
	function widget( $args, $instance ) {
		extract($args);
                ...
                $before_title = '<h3 class="widget-title"><span class="icon icon-calendar"></span>';
		$after_title = '</h3>';
		...
		echo '<div id="calendar_wrap">';
                    get_calendar(false);
		echo '</div>';
                echo '<div class="events_notice">
                        <span class="mark_event"><i class="icon icon-stop"></i> '.__('Posts','tfuse').'</span>
                        <span class="mark_today"><i class="icon icon-stop"></i> '.__('Today','tfuse').'</span>
                    </div>';
		echo $after_widget;
	}
