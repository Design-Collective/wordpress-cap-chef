...
<?php
	...
	function widget( $args, $instance ) {
		...
		$after_widget = '</div>';
		$before_title = ' <h3 class="widget-title"><span class="icon icon-comments-alt"></span>';
		$after_title = '</h3>';
			...
			foreach ( (array) $comments as $comment) {
				...
				$output .=  '<li class="recentcomments clearfix">' . 
                                    /* translators: comments widget: 1: comment author, 2: post link */
                                       ...
                                       '<a href="'.esc_url( get_comment_link($comment->comment_ID) ).'"><span class="thumbnail">'. $avatar .'</span></a>'.
                                       '<span class="recent_comment">
                                        <a href="'.$comment->comment_author_url.'" class="url">'.get_comment_author_link(), '</a>'.$comment->comment_content.'</span><span class="clear"></span>') . 
                                       '</li>';
