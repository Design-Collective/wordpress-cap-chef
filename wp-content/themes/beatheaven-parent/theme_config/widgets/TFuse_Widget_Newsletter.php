...
class TFuse_newsletter extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        <div class="widget-container newsletter_subscription_box newsletterBox">
                <?php if ($newsletter_title != '') { ?><h3 class="widget-title"><span class="icon icon-rss-sign"></span><?php echo tfuse_qtranslate($newsletter_title); ?></h3><?php } ?>
                ...
                <form action="#" method="post" class="newsletter_subscription_form">
                    ...
                    <button type="submit" class="btn btn-primary btn-sm newsletter_subscription_submit" value="<?php _e('Send','tfuse') ?>"><span class="icon-arrow-right"></span></button>
                    <div class="newsletter_subscription_ajax"> <?php _e('Loading...','tfuse') ?></div>
                    <div class="newsletter_text"><?php if ($rss == 'on') { ?><span class="icon-rss"></span>
                        <a href="<?php echo tfuse_options('feedburner_url', get_bloginfo_rss('rss2_url')); ?>" class="link-news-rss">
                            <?php _e('Also subscribe to ', 'tfuse'); ?> <span><?php _e('RSS feed', 'tfuse'); ?></span>
                        </a><?php } ?>
                    </div>
                </form>
        </div>
