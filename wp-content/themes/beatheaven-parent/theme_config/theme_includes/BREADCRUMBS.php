...
function tfuse_breadcrumbs( $args = array() ) {
    global $wp_query, $wp_rewrite, $TFUSE,$tax_name,$post,$is_tf_blog_page,$is_tf_front_page;
    $posttype = $tax_name = $all_blog = '';
    /* Get the textdomain. */
    ...
    if ( is_singular() ) {
        ...
        if( is_single() )
        {
            ...
            if($posttype == 'promo') $tax_name = 'promos';
            if($posttype == 'album') $tax_name = 'albums';
            if($posttype == 'event') $tax_name = 'events';
            if($posttype == 'product') $tax_name = 'products';
            if($posttype == 'date') $tax_name = 'dates';
            
            $terms = get_the_terms($post_id, $tax_name);
            
            if(empty($terms)) $terms = get_the_terms($post_id, 'location');
    ...
    elseif ( is_archive() ) {
        ...
        elseif ( is_time() ) {  
            ...
            if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
                $trail['trail_end'] = get_the_time( __( 'g:i a', 'tfuse' ) );

            elseif ( get_query_var( 'minute' ) )
                $trail['trail_end'] = sprintf( __( 'Minute %1$s', 'tfuse' ), get_the_time( __( 'i', 'tfuse' ) ) );

            elseif ( get_query_var( 'hour' ) )
                $trail['trail_end'] = get_the_time( __( 'g a', 'tfuse' ) );
        }
        ...
        elseif ( is_date() ) {
            ...
            if ( is_day() ) {
                $trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'tfuse' ) ) . '">' . get_the_time( __( 'Y', 'tfuse' ) ) . '</a>';
                $trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'tfuse' ) ) . '">' . get_the_time( __( 'F', 'tfuse' ) ) . '</a>';
                $trail['trail_end'] = get_the_time( __( 'j', 'tfuse' ) );
            }

            elseif ( get_query_var( 'w' ) ) {
                $trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'tfuse' ) ) . '">' . get_the_time( __( 'Y', 'tfuse' ) ) . '</a>';
                $trail['trail_end'] = sprintf( __( 'Week %1$s', 'tfuse' ), get_the_time( esc_attr__( 'W', 'tfuse' ) ) );
            }

            elseif ( is_month() ) {  
                $trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'tfuse' ) ) . '">' . get_the_time( __( 'Y', 'tfuse' ) ) .' '. get_the_time( __( 'F', 'tfuse' ) ).'</a>';
                
            }

            elseif ( is_year() ) { 
                $trail['trail_end'] = get_the_time( __( 'Y', 'tfuse' ) );
            }
        }
    }
    ...
    elseif ( is_search() ){
        $sear =  sprintf( __( 'Search results for &quot;%1$s&quot;', 'tfuse' ), esc_attr( get_search_query() ) );
 $trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'tfuse' ), esc_attr( get_search_query() ) );
}
    /* If viewing a 404 error page. */
    elseif ( is_404() )
        $trail['trail_end'] = __( '404', 'tfuse' );
        $err =__( '404','tfuse');
    ...
    if ( is_array( $trail ) ) {
        ...
        $breadcrumb .= join( "<span class='separator'> {$separator} </span>", $trail );
    ...
    if($is_tf_blog_page)
    {
        $choose = tfuse_options('blogpage_category');
        if($choose == 'all') $all_blog = true;
    }
    ...
    if(!empty($term->name)){
        ...
        ?> 
    <?php if($is_tf_front_page):?>
        <a href="<?php echo home_url()?>"><?php _e('Homepage ','tfuse'); ?></a>
    <?php elseif($all_blog): ?>
        <a href="<?php echo home_url()?>"><?php _e('Blogpage ','tfuse'); ?></a>
    <?php else:?>
        <a href="<?php echo home_url()?>"><?php _e('Homepage ','tfuse'); ?></a><span class="separator">></span> <?php echo $breadcrumb; ?>
    <?php endif;?>
        <?php
