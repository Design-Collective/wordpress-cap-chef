...
function tf_reservationform_shortcode($atts){
    ...
    if($form_exists){
        ...
        $out .='<div class="add-comment"><div class="add-comment-title"><h3 id="header_message">'.urldecode($form['header_message']).'</h3></div>';
        ...
        $out.='<div class="contact_form comment-form">
        ...
        foreach($form['input'] as $form_input){
            ...
            {
                if($is_preview){
                    $ewidth=448-$lines[$element_line]+$margin;}
                else
                    $ewidth=448-$lines[$element_line]+$margin;
            }
            else {
        ...
        $out.='<button id="sending" class="btn btn-primary btn-lg btn-send2" style="display: none;"><span class="icon icon-plus-sign"></span>'.__('Sending ...','tfuse').'</button>
             <button type="submit" id="send_reservation" class="btn btn-primary btn-lg btn-submit2" name="submit" ><span class="icon icon-plus-sign"></span>'. urldecode($form['submit_mess']).'</button>
        ...
        $out.='<div class="clear"></div><div class="notice">'.urldecode($form['tf_rf_form_notice']).'</div>';
    }
    else {
...
function res_radio($input,$form_input){
    ...
    $output="<div class='row alignleft input_styled inlinelist ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+14)."px;".$form_input['floating']."'>
...
function res_checkbox($input,$form_input){
    ...
    $output = "<div class='row alignleft input_styled checklist ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+14)."px;".$form_input['floating']."'>
